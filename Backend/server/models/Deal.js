...
const dealSchema = new mongoose.Schema({
  ...
  status: {
    type: String,
    enum: [
      'created',           // Deal created, waiting for counterparty
      'accepted',          // Both parties accepted
      'kyc_pending',       // KYC verification pending
      'documents_pending', // Document upload pending
      'payment_pending',   // Payment deposit pending
      'contract_pending',  // Contract signing pending
      'funds_deposited',   // Funds in escrow
      'in_delivery',       // Item/service being delivered
      'delivered',         // Delivery completed, inspection period
  ...
  workflow: {
    ...
    itemDelivered: {
      ...
      deliveryProof: [String] // URLs to delivery proof images/documents
    },
    buyerConfirmed: {
      completed: { type: Boolean, default: false },
      completedAt: Date,
...
dealSchema.index({ 'dispute.isDisputed': 1 });
